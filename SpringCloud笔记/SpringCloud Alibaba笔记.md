...
1，设置QPS，quest  per second，即每秒的请求数，Sentinel可设置对应端口所能接受的每秒请求数
...
3，关联，指的是接口A访问失败，让其关联的B接口关闭。例，支付接口出错，让下单接口失败，保证不能支付时，下单功能也不让用，防止连坐。

![1661776214782](note-images/1661776214782.png)

4，Warm Up，当一个接口再请求数为0时属于冷却状态，如果突然从这个状态增长到峰值，有可能会直接让服务崩溃，因此由Warm  Up来进行流控，其原理是当请求忽然增多时，逐渐放行请求，过多的请求就阻止，让请求服务接口增加的曲线平缓，从而达到保护服务的目的。
